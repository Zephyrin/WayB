 ...
 */
final class Version20200504152716 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__brand AS SELECT id, name, uri FROM brand');
        $this->addSql('DROP TABLE brand');
        $this->addSql('CREATE TABLE brand (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, created_by INTEGER DEFAULT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, uri VARCHAR(255) NOT NULL COLLATE BINARY, validate BOOLEAN NOT NULL, CONSTRAINT FK_1C52F958DE12AB56 FOREIGN KEY (created_by) REFERENCES fos_user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO brand (id, name, uri) SELECT id, name, uri FROM __temp__brand');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__brand AS SELECT id, name, uri FROM brand');
        $this->addSql('DROP TABLE brand');
        $this->addSql('CREATE TABLE brand (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, uri VARCHAR(255) NOT NULL, description CLOB NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO brand (id, name, uri) SELECT id, name, uri FROM __temp__brand');
        $this->addSql('DROP TABLE __temp__brand');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_1C52F9585E237E06 ON brand (name)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_1C52F958841CB121 ON brand (uri)');
